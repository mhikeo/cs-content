...
#!                           Default: '90000'
#!                           Optional
#! @input	listen_address: Changes the address where the PostgreSQL database listens.
#!                        Optional
  ...
  workflow:
    ...
    - download_configs_to_local_folder:
        do:
           base.run_command:
            - command: >
                ${'scp -i ' + private_key_file + ' ' + username+ '@'+ hostname+':~/temp/*.conf ' + temp_local_folder}
        publish:
            - return_result
            - return_code
            - error_message
    ...
    - upload_local_configuration_file_to_user_folder:
        do:
           base.run_command:
            - command: >
                ${'scp -i ' + private_key_file + ' ' + configuration_file +' ' + username+ '@'+ hostname+':~/temp/'}
        publish:
    ...
    - update_pg_hba_config:
     ...
     # Copy a local configuration file to remote user folder. On windows *.conf  doesn't work.
    - upload_updated_file_from_temp_local_folder_to_user_folder:
        do:
           base.run_command:
            - command: >
               ${'scp -i ' + private_key_file + ' ' + temp_local_folder+'/postgresql.conf ' + username+ '@'+ hostname+':~/temp/ && scp  -i ' + private_key_file + ' '+ temp_local_folder + '/pg_hba.conf ' + username+ '@'+ hostname+':~/temp/'}
        publish:
            - return_result : ${error_message}
            - return_code
        navigate:
